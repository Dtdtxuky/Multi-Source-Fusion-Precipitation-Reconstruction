...
class CMA_Dataset(Data.Dataset):
    def __init__(self, file_path, phase='train', data_size=256, shape=(1501,1751), invalid_path=None) -> None:
        self.phase = phase
    ...
    def get_data_path(self, file_path, invalid_names_file, phase, train_ratio):
        ...
        with open(file_path, 'r') as file:
            for line in file:
                ...
                path, _, _, pre_x, pre_y = line.strip().split(',')
    ...
    def get_satellite_data(self, idx):
        ...
        for i in range(14):
            ...
            try:
                ...
                print(f"Error sat {path} channel {i}: {e}")
                with open("error_paths.txt", "a") as error_file:
                    error_file.write(f"Sat_{path}\n")
                #! 直接将不好的数据输出None就好
                return None
    ...
    def get_land_data(self, idx):
        ...
        for i, suffix in enumerate(land_suffixes):
            ...
            try:
                ...
                print(f"Error land {path} channel {i}: {e}")
                with open("error_paths.txt", "a") as error_file:
                    error_file.write(f"Land_{path}\n")
    ...
    def get_radar_data(self, idx):
        ...
        try:
            ...
            with open("error_paths.txt", "a") as error_file:
                error_file.write(f"Radar_{path}\n")
        return radar_datas
    ...
    def __getitem__(self, idx):
        ...
        for attempt in range(max_attempts):
            ...
            if sat_data is not None:
                ...
                radar_data = self.get_radar_data(idx)
                
                mask = torch.ones_like(oup_data)
                return {'data_samples': oup_data, 'inputs':oup_data, 'file_name':file, 'label_mask': mask, 'loss_wt_mask':mask}
            else:
...
if __name__ == "__main__":
    file_path = '/mnt/petrelfs/xukaiyi/CodeSpace/SplitDataSet/Path/7month_allpath.txt'
