...
class CMA_Dataset(Data.Dataset):
    def __init__(self, file_path, phase='train', data_size=256, invalid_path=None) -> None:
        ...
        self.satlist = [7,8,9,10,11,12,13]
    ...
    def __getitem__(self, idx):
        ...
        for attempt in range(max_attempts):
            ...
            if sat_data is not None:
                ...
                sat_data_used = torch.from_numpy(sat_data[6:, :, :])
                radar_data = torch.from_numpy(self.get_radar_data(idx))
                inp_data = torch.cat((sat_data_used, radar_data), dim=0)
                
                mask = torch.ones_like(oup_data)
                return {'data_samples': oup_data, 'inputs':oup_data, 'label_mask': mask, 'loss_wt_mask':mask}
            else:
