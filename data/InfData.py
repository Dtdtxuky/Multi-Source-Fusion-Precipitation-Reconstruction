...
class CMA_Dataset(Data.Dataset):
    def __init__(self, file_path, phase='train', save_path='/mnt/petrelfs/xukaiyi/CodeSpace/DiT/DataPath/Inf_patch.txt', sat_list=None, data_size=256, shape=(1501,1751), invalid_path=None) -> None:
        self.phase = phase
    ...
    def get_data_path_patch(self, file_path):
        ...
        for img_path in image_paths:
            for y in range(0, self.shape[1], self.data_size):  
                for x in range(0, self.shape[0], self.data_size): 
                    # 计算当前 patch 的结束位置
                    end_x = x + self.data_size
                    end_y = y + self.data_size
                    
                    # 如果超出边界，调整 x 和 y 的位置，使其不会超出边界
                    if end_x > self.shape[0]:
                        x = self.shape[0] - self.data_size
                        end_x = self.shape[0]  # 更新 end_x 为边界位置
                    if end_y > self.shape[1]:
                        y = self.shape[1] - self.data_size
                        end_y = self.shape[1]  # 更新 end_y 为边界位置

                    patch_list.append(f"{img_path},{x},{y}")
