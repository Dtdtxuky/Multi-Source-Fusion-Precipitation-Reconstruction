...
def subprocess_fn(args):
    ...
    if model.use_ceph:
        ...
        model_checkpoint = os.path.join(args.run_dir, 'dif_checkpoint_latest.pth')
    ...
    # model_without_ddp.test_final_pool(test_dataloader, args.pred_len)
    model_without_ddp.patch2all(test_dataloader, args.pred_len)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfgdir',         type = str,     default = '/mnt/petrelfs/xukaiyi/CodeSpace/DiT/ckpt',  help = 'Where to save the results')
    parser.add_argument('--pred_len',       type = int,     default = 10,                                           help = "predict len")
    parser.add_argument('--metrics_type',   type = str,     default='SEVIRSkillScore',                                         help = 'evaluator to test model')
    ...
    parser.add_argument('--mydataset', type=str, default='patch2all', help='resume from cfg file')
