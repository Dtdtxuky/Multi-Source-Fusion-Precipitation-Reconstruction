    ...
    def __init__(self, logger,tblogger,wandb, rank,**params) -> None:
        ...
        for key in sub_model:
            if key == 'EarthFormer_xy':
                ...
                s1 = sum(p.numel() for p in self.model[key].parameters() if p.requires_grad)
                s1_in_millions = s1 / 1e6
                # 格式化并打印结果
                print(f'autoencoder_kl参数: {s1_in_millions:.2f}M')
                ...
                s2 = sum(p.numel() for p in self.model[key].parameters() if p.requires_grad)
                s2_in_millions = s2 / 1e6
                # 格式化并打印结果
                print(f'Casformer参数: {s2_in_millions:.2f}M')
            else:
    ...
    def save_checkpoint(self, epoch, checkpoint_savedir, save_type='save_best', step=0): 
        ...
        if (utils.get_world_size() > 1 and mpu.get_tensor_model_parallel_world_size() == 1) or utils.get_world_size() == 1:
            if save_type == "save_best":
                ...
                checkpoint_path = checkpoint_savedir / 'checkpoint_latest_{}.pth'.format(step)
        else:
            if save_type == "save_best":
                checkpoint_path = checkpoint_savedir / f'checkpoint_best_{mpu.get_tensor_model_parallel_rank()}.pth'
            else:
                checkpoint_path = checkpoint_savedir / f'checkpoint_latest_{mpu.get_tensor_model_parallel_rank()}_{step}.pth'
    ...
    def _iter_trainer(self, train_data_loader, test_data_loader, max_steps):
        ...
        for step in range(self.begin_step, max_steps):
            ...
            for key in self.lr_scheduler:
                ...
                # metric_logger = self.test_all_ranks(test_data_loader, trainstep= step+1, epoch = (step+1) % int(epoch_step))
                # print("测试完成")
                # # metric_logger = self.test(test_data_loader, epoch= step+1)
            ...
            # metrics = str(metric_logger)  
            # metrics = re.sub(r'\s*\([^)]*\)', '', metrics.replace(" ", ""))
            # metrics_dict = {}
            # key, value = metrics.split(":")
            # value = float(value)
            # metrics_dict[key] = float(value)

            # 记录到 TensorBoard
            for metric_name, metric_value in loss.items():
                self.tblogger.add_scalar(f"Train/{metric_name}", metric_value, step)
                if self.rank == 0:  # 确保只在主进程记录
                    self.wandb.log({f"Train/{metric_name}": metric_value}, step=step)
                    lr=self.optimizer[list(self.optimizer.keys())[0]].param_groups[0]["lr"]
                    self.wandb.log({"Train/lr": lr}, step=step)
    
            ## test and save ##
            if (step+1) % int(epoch_step) == 0 or step+1 == max_steps or (self.debug and step >= 2):
                ## test ##
                train_data_type = self.data_type
                self.data_type = torch.float32
                for key in self.sub_model_name:
                ...
                save_flag = cur_epoch % self.save_epoch_interval == 1 or step+1 == max_steps
