...
def subprocess_fn(args):
    ...
    args.cfg_params["mydataset"] = args.mydataset
...
def main(args):
    ...
    run_dir = os.path.join(args.outdir, f'{desc}',f'{current_time}')
    relative_checkpoint_dir = alg_dir + "/" + f'{desc}' + "/" + f'{current_time}'
    ...
    tbpath = run_dir+"/tblog"
    os.makedirs(run_dir, exist_ok=True)
    tb_logger = SummaryWriter(log_dir=tbpath)
    args.cfg_params["tb_logger"] = tb_logger
    ...
    mywandb = wandb.init(
                ...
                name=f"{args.wandb_name}_32*32_experiment_{current_time}",
                config=cfg_params,
                dir='./Experiment')
    
    args.cfg_params["mywandb"] = mywandb
    args.cfg_params["rank"] = args.rank
...
if __name__ == "__main__":
    ...
    parser.add_argument('--outdir',                     type = str,     default='/mnt/petrelfs/xukaiyi/CodeSpace/CasCast_DiT/Experiment',  help = 'Where to save the results')
    parser.add_argument('--cfg', '-c',                  type = str,     default = '/mnt/petrelfs/xukaiyi/CodeSpace/CasCast_DiT/configs/sevir_used/cascast_diffusion_tiny.yaml',      help = 'path to the configuration file')
    ...
    parser.add_argument('--resume_checkpoint', type=str, default='./ckpt/dif_checkpoint_latest.pth', help='resume checkpoint')
    parser.add_argument('--resume_cfg_file', type=str, default=None, help='resume from cfg file')
    parser.add_argument('--mydataset', type=str, default='Dif', help='resume from cfg file')
    parser.add_argument('--wandb_name', type=str, default='large')
