...
class ConfigBuilder(object):
    ...
    def __init__(self, **params):
        ...
        self.mydataset = params.get('mydataset', {})
        self.tblogger = params.get('tb_logger', {})
        self.wandb = params.get('mywandb', {})
    ...
    def get_model(self, model_params = None):
        ...
        if type == 'non_ar_model':
            ...
            model = non_ar_model(self.logger, self.tblogger, self.wandb, self.rank, **params)
        elif type == 'autoencoder_kl_gan_model':
            from models.autoencoder_kl_gan_model import autoencoder_kl_gan_model
            model = autoencoder_kl_gan_model(self.logger, self.tblogger,self.wandb, self.rank, **params)
        elif type == 'latent_compress_model':
            from models.latent_compress_model import latent_compress_model
            model = latent_compress_model(self.logger, self.tblogger,self.wandb, self.rank, **params)
            ...
            model = latent_diffusion_model_eval(self.logger, self.tblogger, self.wandb, self.rank, **params)
        else:
    ...
    def get_dataloader(self, dataset_params = None, split = 'train', batch_size = None, dataloader_params = None, sampler_params = None):
        ...
        if batch_size is None:
            if split == 'train':
                batch_size = self.trainer_params.get('batch_size', 16)
            elif split == 'valid':
        ...
        if split =='train':
            dataset = Data.create_dataset(self.mydataset, split)
        else:
            dataset = Data.create_dataset(self.mydataset, split)
