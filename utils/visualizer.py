...
class sevir_visualizer(object):
    def __init__(self, exp_dir, sub_dir='sevir_train_vis'):
        ...
        self.save_dir = f'{self.exp_dir}/{sub_dir}/{current_time}'
        os.makedirs(exist_ok=True, name=self.save_dir)

        self.client = Client("~/petreloss.conf")
        
    def visualize3(self, label, full_prediction, save_path):
        rgb_colors = [
            (245, 253, 255),
            (111, 239, 255),  # 5
            (95, 207, 239),   # 10
        ...
        fig, axs = plt.subplots(1, 2, figsize=(150, 50), constrained_layout=True)
    ...
    def save_pixel_image_rain(self, pred_image, target_img, epoch, step):
        if (get_world_size() > 1 and mpu.get_data_parallel_rank() == 0) or get_world_size() == 1:  
            ...
            b,_,_,_ = pred_image.shape
            
            for i in range(b):
                pre = np.squeeze(pred_image[i])
                tar = np.squeeze(target_img[i])
                
                renorm_sr = self.reverse_sqrtlog_minmax_norm(pre)
                renorm_hr = self.reverse_sqrtlog_minmax_norm(tar)
                
                self.visualize3(renorm_hr,renorm_sr, path + "/" + f"Epoch_{epoch}_Iter_{step}_pixel_step{i}.png")
            
            return path + "/" + f"Epoch_{epoch}_Iter_{step}_pixel_step0.png"          

    def save_pixel_image_all(self, pred_image, target_img, name):
        if (get_world_size() > 1 and mpu.get_data_parallel_rank() == 0) or get_world_size() == 1:  
            path = self.save_dir
            os.makedirs(path, exist_ok=True)
            self.visualize3(target_img, pred_image, path + "/" + f"{name}.png")
